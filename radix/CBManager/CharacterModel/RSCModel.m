...
-(void)updateCharacteristicWithHandler:(void (^) (BOOL success, NSError *error))handler
{
    ...
    if(RSCCharacter)
    {
        ...
        [[[CBManager sharedManager] myPeripheral] setNotifyValue:YES forCharacteristic:RSCCharacter];
    }
...
-(void)stopUpdate
{
    ...
    if(RSCCharacter)
    {
        if (RSCCharacter.isNotifying)
        {
            ...
            [[[CBManager sharedManager] myPeripheral] setNotifyValue:NO forCharacteristic:RSCCharacter];
        }
    }
}
